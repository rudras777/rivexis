...
def sqlite_certification():
    with tempfile.TemporaryDirectory(prefix='rivexis-backup-') as tmp:
        ...
        with closing(sqlite3.connect(source)) as src, closing(sqlite3.connect(backup)) as dst:src.backup(dst)
        shutil.copy2(backup,restore)
        with closing(sqlite3.connect(restore)) as con:
        ...
        assert integrity=='ok' and marker==1 and tables==55,(integrity,marker,tables)
        print(f'Backup/restore certification: PASS (SQLite procedural gate; 53 logical + 2 runtime tables; sha256={sha256(backup)})')
