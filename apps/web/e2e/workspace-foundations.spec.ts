...
test.describe("workspace foundations",()=>{
  test("workspace switching never reuses history from the previous workspace",async({page})=>{
    ...
    await page.route("**/api/v1/history?**",route=>{
      ...
      const id=workspaceId==="w-alpha"?"analysis-alpha-only":"analysis-beta-only";
      return route.fulfill({status:200,contentType:"application/json",headers:corsHeaders,body:JSON.stringify({items:[{type:"analysis",id,workspace_id:workspaceId,engine_id:"B2",demo:false,created_at:"2026-09-23T00:00:00Z"}]})});
    });

    await page.goto("/workspace/history");
    await expect(page.getByTestId("workspace-history-state")).toContainText("analysis-alpha-only");
    ...
    await expect(page.getByTestId("workspace-history-state")).toContainText("analysis-beta-only");
    await expect(page.getByText("analysis-alpha-only")).toHaveCount(0);
    ...
    await expect(page.getByTestId("workspace-history-state")).toContainText("analysis-alpha-only");
    await expect(page.getByText("analysis-beta-only")).toHaveCount(0);
  ...
  test("provider runtime telemetry follows the active workspace while registry health stays global",async({page})=>{
    await mockTwoWorkspaces(page);
    await page.route("**/api/v1/providers/status?**",route=>route.fulfill({status:200,contentType:"application/json",headers:corsHeaders,body:JSON.stringify({chain:"ethereum",providers:[{provider_id:"shared-registry-provider",status:"UNCONFIGURED",configured:false}]})}));
    await page.route("**/api/v1/providers/runtime?**",route=>{
      const workspaceId=new URL(route.request().url()).searchParams.get("workspace_id");
      const providers=workspaceId==="w-alpha"?{"alpha-runtime-provider":{logical_calls:3,attempts:3,successes:3,failures:0,cache_hits:0,circuit:{state:"CLOSED"}}}:{};
      return route.fulfill({status:200,contentType:"application/json",headers:corsHeaders,body:JSON.stringify({scope:workspaceId,control_backend:"memory",providers})});
    });
