...
def test_standard_decoder_rejects_noncanonical_address_word_high_bits():
    target = "0x5555555555555555555555555555555555555555"
    noncanonical_address_word = "1" * 24 + target[2:]
    calldata = "0xa9059cbb" + noncanonical_address_word + word(123)

    out = decode_common_calldata(calldata)

    assert out["status"] == "MALFORMED_STANDARD_CALLDATA"
    assert out["signature"] == "transfer(address,uint256)"
    assert out["confidence"] == 0
    assert "parameters" not in out


def test_standard_decoder_rejects_noncanonical_bool_word():
    operator = "0x6666666666666666666666666666666666666666"
    calldata = "0xa22cb465" + address_word(operator) + word(2)

    out = decode_common_calldata(calldata)

    assert out["status"] == "MALFORMED_STANDARD_CALLDATA"
    assert out["signature"] == "setApprovalForAll(address,bool)"
    assert out["confidence"] == 0


def test_verified_abi_decoder_rejects_noncanonical_bool_word():
    from rivexis_api.services.evm_decode import decode_verified_abi_calldata, function_selector

    signature = "configure(bool)"
    calldata = function_selector(signature) + word(2)
    abi = [
        {
            "type": "function",
            "name": "configure",
...
def test_call_trace_does_not_promote_malformed_approval_candidate():
    from rivexis_api.services.evm_decode import normalize_call_trace

    operator = "0x7777777777777777777777777777777777777777"
    malformed_approval = "0xa22cb465" + address_word(operator) + word(2)
    trace = {
        "type": "CALL",
        "from": "0x" + "11" * 20,
        "to": "0x" + "22" * 20,
