...
def _observed_freshness(value):
    ...
    # A materially future-dated provider timestamp is not treated as fresh evidence.
...
def _matching_pools(rows: list[dict], data: dict) -> list[dict]:
    ...
    if pool_id:
        return [
            row
            for row in rows
            if str(row.get("pool") or "").strip().lower() == pool_id
        ]
    ...
    if project:
        candidates = [
            ...
            if project in str(row.get("project") or "").lower()
        ]
    if symbol:
        candidates = [
            row
            for row in candidates
            if symbol in str(row.get("symbol") or "").lower()
        ]
    if chain:
        candidates = [
            row
            for row in candidates
            if chain == str(row.get("chain") or "").lower()
        ]
    return candidates
...
def _insufficient_selector(call, matches: list[dict]) -> EngineResult:
    ...
    return EngineResult(
        ...
        summary=(
            f"The supplied F4 selector matched {len(matches)} current DefiLlama yield "
            "pools. Rivexis did not silently choose one strategy by TVL."
        ),
        metrics={
            "matching_pool_count": len(matches),
            "candidate_pool_ids": candidate_ids,
        },
        warnings=[
            "Provide pool_id, or make protocol/project + asset/symbol + chain specific enough to identify exactly one pool."
        ],
...
def _invalid_pool_metric(call, pool: dict, field: str) -> EngineResult:
    return EngineResult(
        ...
        severity=Severity.UNKNOWN,
        summary=(
            f"The selected yield pool does not contain a usable finite {field} value; "
            "Rivexis did not substitute zero for missing or non-finite provider data."
        ),
        metrics={
...
def run_live_f4(data: dict) -> EngineResult:
    ...
    if not isinstance(rows, list):
        return EngineResult(
            ...
            provider_consensus="UNAVAILABLE",
            provider_status=[
                {"provider_id": call.provider_id, "status": "MALFORMED_RESPONSE"}
            ],
            demo=False,
        )
    ...
    if not matches:
        return EngineResult(
            ...
            warnings=["Rivexis did not infer a pool from a missing match."],
    ...
    assumptions = [
        "DefiLlama yield data is external evidence and is not a Rivexis endorsement or independently verified APY.",
        "If the provider payload omits an observation timestamp, Rivexis records freshness as UNKNOWN rather than equating retrieval time with observation time.",
    ]
    missing = [
        "protocol-native contract state and withdrawal constraints",
        "independent smart-contract/security evidence",
        "oracle and dependency validation",
        "historical yield stability beyond provider summary metrics where needed",
    ]
        ...
        try:
            ...
            if native.metrics.get("declared_contracts"):
                missing = [
                    item
                    for item in missing
                    if item != "protocol-native contract state and withdrawal constraints"
                ]
                missing.append(
                    "withdrawal constraints and strategy semantics beyond declared contract state"
                )
            if native.metrics.get("declared_oracle"):
                missing = [
                    item
                    for item in missing
                    if item != "oracle and dependency validation"
                ]
                missing.append(
                    "independent verification of declared oracle/dependency authority"
                )
        except (ValueError, ProviderError) as exc:
            warnings.append(
                f"Protocol-native yield dependency evidence could not be collected: {exc}"
            )
            missing.append("protocol-native yield dependency evidence")

    if freshness in {FreshnessStatus.STALE, FreshnessStatus.EXPIRED}:
        warnings.append(
            "Yield-pool observation is stale; refresh APY/TVL evidence before strategy decisioning."
        )
    consensus = (
        "MULTI_SOURCE" if len({item.provider for item in all_evidence}) > 1 else "SINGLE_SOURCE"
    )
    status = (
    ...
    return EngineResult(
        ...
        summary=(
            f"Current attributed yield evidence found for {pool.get('project') or 'protocol'} / "
            f"{pool.get('symbol') or 'pool'}: {apy:.2f}% headline APY. Protocol-native "
            "evidence was incorporated when declared; unresolved principal-risk gaps remain explicit."
        ),
        metrics=metrics,
        signals=signals,
        warnings=warnings,
        mitigations=mitigations,
        safer_alternatives=mitigations[:1],
        evidence=all_evidence,
        provider_consensus=consensus,
        data_freshness={
            "status": freshness.value,
            "provider": "multi-source" if consensus == "MULTI_SOURCE" else "defillama_yields",
            "age_seconds": age_seconds,
            "provider_timestamp_present": pool.get("timestamp") not in (None, ""),
        },
