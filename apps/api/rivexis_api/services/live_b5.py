...
def _evidence(call: ProviderCall, normalized: Any) -> EvidenceRecord:
    return EvidenceRecord(
        ...
        calculation_version="b5-live-1.2.0",
    ...
    *,
    ...
    if estimate_from_amount not in (None, ""):
        ...
        if parsed_estimate_from != amount:
            conflicts.append(
                _conflict("route.estimate_from_amount", str(amount), estimate_from_amount)
            )

    quoted_from_address = _evm_address(action.get("fromAddress"))
    if quoted_from_address != from_address:
        conflicts.append(
            _conflict("route.from_address", from_address, action.get("fromAddress"))
        )
    ...
    if not _token_matches(from_token, action.get("fromToken")):
        conflicts.append(
            _conflict("route.from_token", str(from_token), action.get("fromToken"))
        )
    ...
    if quoted_slippage not in (None, ""):
        try:
            if abs(float(quoted_slippage) - slippage) > 1e-12:
    ...
    if to_amount_min_raw not in (None, "") and minimum is None:
        conflicts.append(
            _conflict("route.minimum_to_amount", "positive integer", to_amount_min_raw)
        )
    if to_amount is not None and minimum is not None and minimum > to_amount:
        conflicts.append(
            _conflict(
                "route.minimum_not_above_expected",
                f"<= {to_amount}",
                minimum,
            )
        )
...
def run_live_b5(input_data: dict[str, Any]) -> EngineResult:
    try:
        source = normalize_chain(
            input_data.get("source_chain") or input_data.get("fromChain")
        )
        destination = normalize_chain(
            input_data.get("destination_chain") or input_data.get("toChain")
        )
    ...
    if slippage < 0 or slippage > 0.5:
        return _fail("slippage must be between 0 and 0.5")
    if not all([from_token, to_token, amount_raw, wallet_raw]):
        return _fail(
            "B5 live analysis requires source/destination chain, token, amount in smallest units, and wallet"
        )
    ...
    tool_details = (
        body.get("toolDetails") if isinstance(body.get("toolDetails"), dict) else {}
    )
    ...
    normalized = {
        ...
        "from_token": (
            (action.get("fromToken") or {}).get("symbol")
            if isinstance(action.get("fromToken"), dict)
            else None
        ),
        "to_token": (
            (action.get("toToken") or {}).get("symbol")
            if isinstance(action.get("toToken"), dict)
            else None
        ),
        ...
        "gas_cost_usd": _usd_total(estimate.get("gasCosts")),
        "fee_cost_usd": _usd_total(estimate.get("feeCosts")),
    ...
    if conflicts:
        ...
        return EngineResult(
            ...
            severity=Severity.UNKNOWN,
            summary=(
                "LI.FI returned a route quote that does not match the requested route or contains internally inconsistent amount bounds. Rivexis discarded route scoring."
            ),
            metrics=normalized,
            warnings=[
                "Do not sign or execute this quote until request/response integrity conflicts are resolved."
            ],
            ...
            missing_data=["request-consistent cross-chain route quote"],
            provider_status=provider_status,
            assumptions=[
                "Rivexis treats caller route parameters as the trust-boundary request contract and does not silently accept a materially different provider response."
            ],
        )
    ...
    if source.chain_id != destination.chain_id:
        score += 8
    warnings = [
        "Route quote is aggregator evidence, not an independent bridge-security assessment."
    ]
    missing = [
    ...
    if estimate.get("toAmountMin") in (None, ""):
        ...
        warnings.append(
            "LI.FI response did not provide a normalized minimum received amount."
        )
    return EngineResult(
        engine_id=EngineId.B5,
        engine_version="1.2.0",
        ...
        severity=_sev(score),
        summary=(
            "A live LI.FI route quote matched the requested route parameters and was normalized. Independent bridge-security/liquidity validation remains incomplete, so the route remains a partial analysis."
        ),
        metrics=normalized,
        warnings=warnings,
        mitigations=[
            "Cross-check bridge security and liquidity before signing a high-value cross-chain transaction."
        ],
        safer_alternatives=[
            "Request and compare additional routes after independent security providers are connected."
        ],
        evidence=evidence,
