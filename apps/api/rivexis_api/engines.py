...
def _b3_snapshot_contract_error(input_data: dict) -> EngineResult | None:
    ...
    try:
        ...
        return None
    previous_chain_id = previous.get("chain_id")
    try:
        previous_chain_id_int = int(previous_chain_id)
    except (TypeError, ValueError):
        return _b3_snapshot_failure(
            "previous_snapshot is missing a valid chain_id identity"
        )
    if previous_chain_id_int != current_chain.chain_id:
        return _b3_snapshot_failure(
    ...
    previous_oracle_obj = previous.get("oracle")
    previous_oracle = (
        str(previous_oracle_obj.get("feed") or "").strip().lower()
        if isinstance(previous_oracle_obj, dict)
        else ""
    )
...
def _score(engine_id, d):
    ...
    if engine_id == EngineId.F1:
        ...
        return min(100, max(10, concentration) + (25 if d.get("stablecoin_concentration", 0) > 0.7 else 0))
    ...
    if engine_id == EngineId.F4:
        ...
        return min(100, 20 + (25 if apy > 30 else 0) + (30 if d.get("incentive_dependent") else 0) + (25 if d.get("lockup") else 0))
    if engine_id == EngineId.F5:
        concentration = float(d.get("largest_allocation_pct", 25))
        return min(100, max(15, concentration) + (30 if d.get("stablecoin_depeg_scenario_loss_pct", 0) > 20 else 0))
    return 0
...
def _run_live(engine_id: EngineId, input_data: dict) -> EngineResult:
    if engine_id == EngineId.B1:
        ...
        result = _enforce_explicit_f3_adapter_contract(run_live_f3(input_data), input_data)
...
def run_engine(engine_id: EngineId, input_data: dict, demo: bool = False):
    ...
    return EngineResult(
        ...
        severity=_sev(score),
        summary=f"Demonstration result for {ENGINE_NAMES[engine_id]}: detected risk score {score:.0f}/100.",
