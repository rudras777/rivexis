...
|---|---:|---|---|
| B1 — Transaction Simulation | 1.3.0 | EVM RPC state/dry-run; mined receipts; optional `debug_traceCall`; Tenderly when configured; optional verified ABI lookup | Calculation `b1-live-1.6.0`. Standard ERC event effects remain validated/log-grounded. Static and supported composite ABI decoding remains canonical and bounded. Internal call traces validate canonical quantities/calldata/addresses, enforce depth/node limits and cycle detection, report malformed/discarded/truncated coverage, reduce confidence when partial, and do not promote malformed/unavailable nodes into transfer or approval evidence. Tuple arrays, nested tuples and dynamic tuple members remain explicit `UNSUPPORTED_VERIFIED_ABI_TYPE`. Deeper state/security semantics remain incomplete. |
...
**B1.** Standard ERC-20/ERC-721/ERC-1155 effects come only from raw logs whose standard topic signature and ABI layout validate. Receipt logs for mined transactions are observed evidence; Tenderly simulation logs are predicted. Non-canonical static calldata remains malformed rather than coerced. Verified-ABI dynamic `bytes`/`string` offsets must be 32-byte aligned, point outside the fully expanded static head, contain complete bounded tail data, and use zero ABI padding. Supported static-element dynamic arrays inherit canonical element validation and a bounded item limit. One-dimensional fixed arrays and top-level static tuples of the same supported elementary types expand into their exact ABI head width, validate every element/component and enforce bounded item counts. Optional call-trace evidence separately validates every address, calldata value and JSON-RPC uint256 quantity, bounds traversal, detects cycles and labels partial/unavailable normalization; only valid addresses and quantities can become promoted transfer/approval candidates. Tuple arrays, nested tuples, dynamic tuple members and other nested composites are not guessed.
