...
|---|---:|---|---|
| B1 — Transaction Simulation | 1.3.0 | EVM RPC state/dry-run; mined receipts; optional `debug_traceCall`; Tenderly when configured; optional verified ABI lookup | Calculation `b1-live-1.5.0`. Standard ERC event effects remain validated/log-grounded. Static ABI decoding enforces canonical address/bool/integer/fixed-bytes encodings. Verified-ABI dynamic `bytes`/`string`, dynamic arrays, bounded one-dimensional fixed arrays and top-level static tuples of supported elementary types validate offsets, expanded head/tail bounds, zero padding and canonical elements. Tuple arrays, nested tuples and dynamic tuple members remain explicit `UNSUPPORTED_VERIFIED_ABI_TYPE`. Deeper internal/state/security semantics remain incomplete. |
