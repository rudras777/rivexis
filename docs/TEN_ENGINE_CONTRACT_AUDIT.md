...
|---|---:|---|---|
| B1 — Transaction Simulation | 1.3.0 | EVM RPC state/dry-run; mined receipts; optional `debug_traceCall`; Tenderly when configured; optional verified ABI lookup | Standard ERC event effects remain validated/log-grounded. ABI-free and verified-ABI decoding enforce canonical address/bool/integer/fixed-bytes encodings; malformed approval-shaped calldata is not promoted into an approval candidate. Deeper internal/state/security semantics remain incomplete. |
...
**B1.** Standard ERC-20/ERC-721/ERC-1155 effects come only from raw logs whose standard topic signature and ABI layout validate. Receipt logs for mined transactions are observed evidence; Tenderly simulation logs are predicted. Non-canonical static calldata remains malformed rather than coerced.
