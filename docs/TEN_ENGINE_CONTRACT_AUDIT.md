...
|---|---:|---|---|
| B1 — Transaction Simulation | 1.3.0 | EVM RPC state/dry-run; mined receipts; optional `debug_traceCall`; Tenderly when configured; optional verified ABI lookup | Calculation `b1-live-1.4.0`. Standard ERC event effects remain validated/log-grounded. Static ABI decoding enforces canonical address/bool/integer/fixed-bytes encodings. Verified-ABI dynamic `bytes`/`string`, dynamic arrays and bounded one-dimensional fixed arrays of supported one-word static elementary types validate offsets, expanded head/tail bounds, zero padding and canonical elements. Unsupported tuple and nested/composite layouts remain explicit `UNSUPPORTED_VERIFIED_ABI_TYPE`. Deeper internal/state/security semantics remain incomplete. |
...
**B1.** Standard ERC-20/ERC-721/ERC-1155 effects come only from raw logs whose standard topic signature and ABI layout validate. Receipt logs for mined transactions are observed evidence; Tenderly simulation logs are predicted. Non-canonical static calldata remains malformed rather than coerced. Verified-ABI dynamic `bytes`/`string` offsets must be 32-byte aligned, point outside the fully expanded static head, contain complete bounded tail data, and use zero ABI padding. Supported static-element dynamic arrays inherit canonical element validation and a bounded item limit. One-dimensional fixed arrays of the same supported elementary types expand into their exact ABI head width, validate every element and enforce the same item bound. Tuple and nested/composite layouts are not guessed.
