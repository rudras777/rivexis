...
| B4 — Entity & Fund Flow | 1.0.0 | Direct wallet state; optional Etherscan indexed history; optional Nansen; Arkham only when license-approved | `UNKNOWN ADDRESS` is preserved when attribution is absent; conflicting external labels remain unresolved instead of silently choosing a label. |
...
After this cross-engine contract normalization is CI-certified, the highest-priority engine-specific follow-up is F3: when a caller explicitly asks for a protocol-specific adapter and that authoritative path fails, the failure should be surfaced explicitly rather than being silently discarded before the generic modeled-oracle path is considered.
