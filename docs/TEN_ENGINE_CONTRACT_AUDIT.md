...
|---|---:|---|---|
| B1 — Transaction Simulation | 1.3.0 | EVM RPC state/dry-run; mined receipts; optional `debug_traceCall`; Tenderly when configured; optional verified ABI lookup | Standard ERC event effects remain validated/log-grounded. ABI-free and verified-ABI static decoding enforce canonical address/bool/integer/fixed-bytes encodings. Verified-ABI dynamic `bytes`/`string` and dynamic arrays of supported one-word static elementary types are bounded and canonical: offsets, head/tail bounds, padding and array elements validate. Unsupported tuples, fixed arrays and nested/composite dynamic types remain explicit `UNSUPPORTED_VERIFIED_ABI_TYPE` rather than plausible raw-word decoding. Deeper internal/state/security semantics remain incomplete. |
| B2 — Transaction & Contract Security | 1.1.0 | EVM RPC bytecode/calldata rules; optional Etherscan; Blockaid when resolved | Calculation generation `b2-live-1.2.0`. Target/sender/hash validation is retained; bytecode is validated and read at the captured RPC block; approval analysis reuses canonical calldata decoding so malformed address padding/bool words cannot become plausible permissions. External threat depth remains partial when approved providers are unavailable. |
...
| B4 — Entity & Fund Flow | 1.0.0 | Direct wallet state; optional Etherscan history; optional Nansen; Arkham only when license-approved | Direct native balance is pinned to the captured RPC block. Malformed indexed state is excluded, ERC-20 identity is contract-based, conflicting decimals are skipped, and concentration is descriptive rather than maliciousness/economic-ownership evidence. External indexed/entity evidence currently has UNKNOWN provider observation time; removing misleading current-block/aggregate-LIVE provenance from those rows is the next audited deterministic target. Cross-chain and richer protocol/counterparty attribution remain incomplete. |
...
| F1 — Portfolio & Exposure | 1.2.0 | CoinGecko references; direct native/ERC-20 reads for declared assets; manual positions | Calculation `f1-live-1.3.0`. One captured block pins all requested native/ERC-20 reads; explicit holdings remain all-or-nothing and positive exposures require complete positive finite prices. Automatic discovery, NFTs and DeFi positions remain gaps. |
...
**B1.** Standard ERC-20/ERC-721/ERC-1155 effects come only from raw logs whose standard topic signature and ABI layout validate. Receipt logs for mined transactions are observed evidence; Tenderly simulation logs are predicted. Non-canonical static calldata remains malformed rather than coerced. Verified-ABI dynamic `bytes`/`string` offsets must be 32-byte aligned, must point outside the static head, must contain complete bounded length/tail data, and must use zero ABI padding. Supported static-element dynamic arrays inherit canonical element validation and a bounded item limit. Tuple, fixed-array and nested/composite dynamic layouts are not guessed; unsupported layouts return an explicit unsupported state with zero confidence.

**B2.** Input and RPC-resolved addresses/hashes validate before downstream use. Contract code must be valid hex and is observed at the same captured block referenced by its evidence. Approval-shaped calldata follows the same canonical address/bool boundary as B1 rather than using permissive truthiness/padding rules.
...
**B4.** Direct-chain and external-provider evidence stay distinct. Native-balance state is pinned to the captured block. Token identity is contract-based, malformed rows are excluded, and counterparty concentration is descriptive interaction concentration—not ownership, maliciousness or economic exposure. External indexer/entity-label responses without provider observation timestamps remain UNKNOWN-freshness evidence and must not inherit a current RPC block as though they were block-specific observations.
...
**F1.** Explicit wallet/token reads are all-or-nothing. All direct holdings reads in one live snapshot share the captured block tag. Positive exposures require complete positive finite prices before weights/concentration scoring and market freshness requires credible timestamp coverage.
...
- `bab6bf2b56fbdef7bfc059f9e6e5c169da567838` / `e285b83af7d3a70f4052d9c8e41d298456869bf6` — B1 verified-ABI dynamic decoder + focused regressions — included in certified state `149f37d6243d435eb6d42f106e3d1b14b899e2e7`, CI #349 (`35933475754`) PASS across API, web/E2E, invariants and PostgreSQL migration/runtime-control jobs.

## Remaining Milestone F work

Deterministic integrity defects identified in these campaigns are regression-covered. Remaining work is primarily capability depth: deeper B1 internal/state/security semantics plus tuple/fixed/nested ABI decoding only where justified; approved independent B2/B3 security/threat evidence and continuous monitoring; B4 external indexed/entity evidence provenance plus cross-chain/protocol-semantic attribution; F1 automatic/indexed discovery plus NFT/DeFi positions; independent B5 bridge-security/liquidity/incident evidence; and deeper F2/F4/F5 dependency/liquidity/governance/counterparty/strategy evidence. Credential/licensing/customer-contract gaps stay explicit and must not be replaced by synthetic success.
