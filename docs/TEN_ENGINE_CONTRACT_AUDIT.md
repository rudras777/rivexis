...
|---|---:|---|---|
| B1 — Transaction Simulation | 1.3.0 | EVM RPC state/dry-run; mined receipts; optional `debug_traceCall`; Tenderly when configured; optional verified ABI lookup | Calculation `b1-live-1.7.0`. Standard ERC event effects, ABI decoding and internal-call normalization remain canonical/bounded. Prestate diffs validate addresses, account objects, balances, uint64 nonces, bytecode and storage words; honor sparse Geth `post` semantics; label account creation/deletion; cap address/storage/output work; and surface partial/unavailable evidence at reduced/zero confidence. Tuple arrays, nested tuples and dynamic tuple members remain explicit `UNSUPPORTED_VERIFIED_ABI_TYPE`. Deeper security semantics remain incomplete. |
...
**B1.** Standard ERC-20/ERC-721/ERC-1155 effects come only from raw logs whose standard topic signature and ABI layout validate. Receipt logs for mined transactions are observed evidence; Tenderly simulation logs are predicted. Non-canonical static calldata remains malformed rather than coerced. Verified-ABI supported composites and optional call traces remain canonical and bounded; only valid addresses and quantities become promoted transfer/approval candidates. Prestate diffs separately validate each address/account/basic field/storage word, follow Geth's contract that unchanged basic fields are omitted from `post`, and distinguish modified, created and deleted accounts. Malformed or capped state is explicit partial/unavailable evidence and is excluded from canonical change summaries. Tuple arrays, nested tuples, dynamic tuple members and other nested composites are not guessed.
