...
- **B1:** engine `1.3.0`, calculation `b1-live-1.5.0`. Standard event effects are validated/log-grounded; canonical address/bool/integer/fixed-bytes ABI rules prevent malformed permission-shaped calls from becoming plausible approvals. Verified-ABI dynamic `bytes`/`string`, dynamic arrays, bounded one-dimensional fixed arrays and top-level static tuples of supported elementary types validate alignment, expanded static-head boundaries, tail lengths, padding and elements. Tuple arrays, nested tuples and dynamic tuple members remain explicit unsupported layouts.
...
- B1 deeper internal-call/state/security semantics; nested/dynamic tuple and other composite ABI decoding only where it materially improves transaction understanding and can remain bounded/canonical.
