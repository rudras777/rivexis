...
| In-place workspace switching across all tools | BLOCKED | A hard reload remains intentionally enforced because Monitors, Investigations, Protocol History and other legacy tool/result surfaces still hold workspace-local component state. Those surfaces must be migrated before reload can be removed safely. |
| Current milestone CI | PASS | PR #6 CI run #93 (`35799257420`) passed API, web/Playwright/Axe, invariants/secret scan and PostgreSQL migration/runtime-control jobs. |
