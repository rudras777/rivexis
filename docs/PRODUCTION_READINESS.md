...
| Analysis/report provenance presentation | PASS | PR #11 carries canonical provenance into decision HTML/PDF and structured engine-result UI. Zero-evidence provider failures, partial/conflicting/stale states, demo status, evidence providers/count, versions and missing data are rendered from returned fields; raw JSON remains available only as a disclosure. |
| Persisted History provenance | NOT STARTED | Workspace History remains identity/time focused and still needs canonical analysis status/version/evidence metadata plus decision state/methodology where supported before Milestone E can be considered complete. |
...
| Current milestone CI | PASS | PR #11 CI run #154 (`35831726073`) passed API, web/Playwright/Axe, invariants/secret scan and PostgreSQL migration/runtime-control jobs. |
